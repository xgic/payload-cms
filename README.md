...
```text
ghcr.io/xgic/payload-cms-dev:0.3.5
```
...
| **Reproducible pins** | Semver image tags (e.g. `0.3.5`) match producer releases |
...
VS Code starts the **Docker Compose** project in `.devcontainer/docker-compose.yml` (service `xgic-payload-cms-dev`, default project name `xgic-payload-cms-dev`) and **pulls** `ghcr.io/xgic/payload-cms-dev:0.3.5` (first pull may take several minutes). Postgres starts with the app. Do **not** use a standalone `image` in `devcontainer.json` — that produces non-deterministic container names (Docker’s default `adjective_noun` style) and detaches the DB from the IDE project.
...
```bash
# Postgres is already up after Reopen. Use `xgic up` only for CLI-driven
# bring-up (for example outside the IDE). Mongo is still profiled:
#   xgic up --profile mongodb
...
| Tag | Use when |
|-----|----------|
| `:0.3.5` (semver) | **Default for app work** — matches producer [v0.3.5](https://github.com/xgic/payload-cms-dev/releases/tag/v0.3.5) |
...
| Symptom | What to try |
|---------|-------------|
| Image pull fails | Confirm Docker is running; `docker pull ghcr.io/xgic/payload-cms-dev:0.3.5`; check [package page](https://github.com/users/xgic/packages/container/package/payload-cms-dev) |
...
| Git `safe.directory` / SSH agent in the container | Image Git DX (`:0.3.5` entrypoint). Prefer HTTPS. Optional overlay: `.devcontainer/docker-compose.git-dx.yml`. Not a `postStartCommand` |
...
| Host-conditional Git DX | This template (Compose start); producer [payload-cms-dev#49](https://github.com/xgic/payload-cms-dev/issues/49) |
